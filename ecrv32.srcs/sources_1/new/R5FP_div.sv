...
module R5FP_div #(
	...
	localparam ExtWidth=(SIG_W%2==1)? (SIG_W + 3) : (SIG_W + 4) ) (
		input  wire [SIG_W + EXP_W:0] a_i, b_i,
		input  wire [2:0] rnd_i,
		input wire strobe_i,
		output wire [EXP_W-1:0] xExp_o,tailZeroCnt_o,
		output wire [SIG_W+3-1:0] xSig_o,
		output wire [5:0] xMidStatus_o,

		output wire [7:0] xStatus_fast_o,
		output wire  [SIG_W + EXP_W:0] x_fast_o,
		output wire x_use_fast,
		output wire [2:0] rnd_o,

		output wire [ExtWidth-1:0] idiv_N, 
		output wire [ExtWidth-1:0] idiv_D, 
		output wire idiv_strobe,
		input wire [ExtWidth-1:0] idiv_Quo, 
		input wire [ExtWidth-1:0] idiv_Rem,
		input wire idiv_done,
		input wire idiv_ready,

		output wire done_o, wire ready_o,
